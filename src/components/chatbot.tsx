    ...
    if (isOpen && messages.length === 0) {
      ...
      setTimeout(() => {
        setMessages([
          {
            role: "assistant",
            content: "My name is Amirul Maulana. What can I help you with?",
          },
        ]);
        setIsLoading(false);
      }, 1000);
    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        <Button
          variant="default"
          size="lg"
          className="fixed bottom-6 right-6 z-50 h-16 w-16 rounded-full shadow-lg bg-black text-white hover:bg-white hover:text-black hover:shadow-[0_0_12px_2px_rgba(255,255,255,0.7)] transition-all duration-300 transform hover:scale-105 border border-white"
        >
          <AnimatePresence>
            {isOpen ? (
              <motion.div
      ...
      >
        <div className="p-4 border-b border-border/50">
          ...
          <p className="text-sm text-muted-foreground">
            Your friendly portfolio assistant
          </p>
        </div>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <Button
                  ...
                  onClick={() =>
                    handlePromptClick(examplePrompts[currentPromptIndex])
                  }
                >
